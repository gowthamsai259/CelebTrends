  ...
  return (
    <div className="min-h-screen bg-gradient-hero">
      ...
      <div className="container mx-auto px-4 pb-12">
                  ...
                  <CardContent className="p-6">
                    <div className="grid md:grid-cols-2 gap-6">
                      {trendingCelebrities.map((celebrity, index) => (
                        <div key={index} className="group p-6 bg-background/30 backdrop-blur-sm border border-border/50 rounded-xl hover:shadow-glow transition-all duration-500 hover:border-primary/50 animate-float" style={{ animationDelay: `${index * 0.1}s` }}>
                          <div className="flex justify-between items-start mb-4">
                            <div>
                              <h3 className="text-xl font-bold text-foreground group-hover:text-primary transition-colors">
                                #{index + 1} {celebrity.name}
                              </h3>
                              <span className="text-sm text-muted-foreground">{celebrity.category}</span>
                            </div>
                            <div className="text-right">
                              <div className="bg-gradient-primary text-primary-foreground px-3 py-1 rounded-full text-sm font-semibold">
                                {celebrity.searchCount}
                              </div>
                          ...
                          <div className="space-y-2">
                            ...
                            {celebrity.questions.map((question, qIndex) => (
                              <div 
                                key={qIndex} 
                                className="text-sm text-muted-foreground cursor-pointer hover:text-primary transition-colors p-2 rounded-lg hover:bg-primary/5 border border-transparent hover:border-primary/20"
                    ...
                    {selectedAnswer && (
                      <div className="mt-8 p-6 bg-gradient-primary/10 backdrop-blur-sm rounded-xl border border-primary/20 animate-fade-in">
                        <h4 className="font-semibold mb-3 text-primary flex items-center gap-2">
                          <Star className="h-4 w-4" />
                          Detailed Answer:
                        </h4>
                ...
                <Card className="bg-gradient-card shadow-card border-0">
                  <CardHeader className="text-center bg-gradient-primary/10 rounded-t-lg">
                    <CardTitle className="flex items-center justify-center gap-2 text-2xl">
                      ...
                      Top 10 Most Searched Websites Globally
                    </CardTitle>
                    <p className="text-muted-foreground">The most visited and searched websites across the internet</p>
                  </CardHeader>
                  <CardContent className="p-6">
                    <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-4">
                      {topWebsites.map((website, index) => (
                        <div key={index} className="group flex items-center justify-between p-4 bg-background/30 backdrop-blur-sm rounded-xl hover:shadow-glow transition-all duration-300 border border-border/50 hover:border-primary/50 animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
        ...
        {/* Bottom Ad Space */}
        <GoogleAd size="728x90 Leaderboard" className="mt-8" />
      </div>
    </div>
  );
};
